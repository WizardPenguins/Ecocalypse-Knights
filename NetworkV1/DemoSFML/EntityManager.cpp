...
EntityManager::EntityManager(sf::Sprite _poison, sf::Sprite _slash)
{
	this->tempImage = _poison.getTexture()->copyToImage();
	this->size = _poison.getTexture()->getSize();
	this->oneFrameSize = { (float)(size.x / 6), (float)(size.y / 8) };

	this->dimSlash = sf::Vector2f{ _slash.getTexture()->getSize() };
	this->dimSlashOneFrame = sf::Vector2f{ (float)(this->dimSlash.x / 46),270.f };
	this->imageSlash = _slash.getTexture()->copyToImage();
}
			...
			for (int i = 0; i < _mob.size(); i++)
			{
				if (_mob[i]->GetState() != MobState::Death && _mob[i]->GetState() != MobState::Dispawn && TestWitch(_mob[i]))
				{
						...
						_projectile[k].posAutor.y <= _mob[i]->GetPos().y + DistAxeY && _projectile[k].posAutor.y >= _mob[i]->GetPos().y - DistAxeY)
					{
						...
						_mob[i]->SetDirection(_mob[i]->Trigo(_projectile[k].pos));
...
#pragma endregion				
					...
					if (_player[i]->GetHeavyAttackActived() && Utils::AABB_AABB_Collision(_player[i]->GetHeavyAttack(), _mob[j]->GetHitBox()) && _mob[j]->GetType() != TypeMob::Boss &&
						_player[i]->GetPos().y <= _mob[j]->GetPos().y + DistAxeY && _player[i]->GetPos().y >= _mob[j]->GetPos().y - DistAxeY)
					{
...
void EntityManager::MobArrowCollision(std::vector<Projectile>& _projectile, std::vector<Scarecrow*>& _scarecrow, Audio& _audio)
{
	for (int k = 0; k < _projectile.size(); k++)
	{
		if (_projectile[k].autor == AutorProjectile::PLAYER)
				...
				if (diffSlashPlayerPos.x >= 0 && diffSlashPlayerPos.y >= 0)
				{
					if ((testValue.x > this->dimSlashOneFrame.x * slashInfo.x) && (testValue.x < this->dimSlashOneFrame.x * slashInfo.x + this->dimSlashOneFrame.x))
					{
						if ((testValue.y > 0) && (testValue.y < this->dimSlashOneFrame.y))
						{
							...
							{
								_tabPlayer[i]->AddLife(0);
								_tabPlayer[i]->SetPlayerState(HIT);
							}
						}
					}
				}
...
void EntityManager::UpdateWitchHitBox(Witch* _witch, std::vector<Player*>& _player, Audio& _audio)
{
	...
	if (_witch->GetHasSpawnPlant())
	{
		for (int i = 0; i < _witch->GetVectorPlant().size(); i++)
		{
			...
			else if (_witch->GetVectorPlant()[i]->isReady && _witch->GetVectorPlant()[i]->eatSomeone && _witch->GetVectorPlant()[i]->spitOut)
			{
...
void EntityManager::DrawBossBar(std::string _race, HudBarBoss& _spriteBoss, Mob& _mob, Wave& _wave, sf::RenderWindow& _window, float _decalage, sf::Font _font, std::string _string)
{
	if (_mob.GetState() != MobState::Death)
	{
		if (!this->bossBarInit)
		{
			...
			this->textTaMere.setPosition(sf::Vector2f{ (float)(WORKING_WIDTH / 2 + this->waveBossBar * WORKING_WIDTH),1020.f });
		...
		sf::Vector2f posLifeBar = sf::Vector2f{ (float)(WORKING_WIDTH / 2 + this->waveBossBar * WORKING_WIDTH),1020.f };
...
void EntityManager::UpdateGoldVector(float _dt)
{
	for (int i = 0; i < goldHud.size(); i++)
	{
		if (goldHud[i]->timerEasing < 1)
		{
			...
			goldHud[i]->goldSprite.setPosition({ goldHud[i]->pos.x - 50, goldHud[i]->pos.y });
		}
		else
		{
...
void EntityManager::DisplayDamageVector(sf::RenderTexture& _red, sf::RenderTexture& _blue)
{
	for (int i = 0; i < damageHud.size(); i++)
	{
		...
		damageHud[i]->damageBack.setPosition(pos2.x + (TROAD_GAP * (-1 * pos2.y + WORKING_HEIGHT) / 100.f), pos2.y);
		_red.draw(damageHud[i]->damageBack);

		damageHud[i]->damageBack.setPosition(pos2.x - (TROAD_GAP * (-1 * pos2.y + WORKING_HEIGHT) / 100.f), pos2.y);
		_blue.draw(damageHud[i]->damageBack);

		damageHud[i]->damageBack.setPosition(pos2);

		if (damageHud[i]->displayHeart)
		{
			sf::Vector2f pos3 = damageHud[i]->heartSprite.getPosition();

			damageHud[i]->damageBack.setPosition(pos3.x + (TROAD_GAP * (-1 * pos3.y + WORKING_HEIGHT) / 100.f), pos3.y);
			_red.draw(damageHud[i]->damageBack);

			damageHud[i]->damageBack.setPosition(pos3.x - (TROAD_GAP * (-1 * pos3.y + WORKING_HEIGHT) / 100.f), pos3.y);
			_blue.draw(damageHud[i]->damageBack);

			damageHud[i]->damageBack.setPosition(pos3);
		}
	}
}
...
void EntityManager::DisplayGoldVector(sf::RenderTexture& _red, sf::RenderTexture& _blue)
{
	for (int i = 0; i < goldHud.size(); i++)
	{
		...
		goldHud[i]->goldBack.setPosition(pos2.x + (TROAD_GAP * (-1 * pos2.y + WORKING_HEIGHT) / 100.f), pos2.y);
		_red.draw(goldHud[i]->goldBack);
		...
		goldHud[i]->goldBack.setPosition(pos3.x + (TROAD_GAP * (-1 * pos3.y + WORKING_HEIGHT) / 100.f), pos3.y);
		_red.draw(goldHud[i]->goldBack);

		goldHud[i]->goldBack.setPosition(pos3.x - (TROAD_GAP * (-1 * pos3.y + WORKING_HEIGHT) / 100.f), pos3.y);
		_blue.draw(goldHud[i]->goldBack);

		goldHud[i]->goldBack.setPosition(pos3);
...
void EntityManager::FillVectorSort(std::vector<Projectile>& _projectile, std::vector<Player*>& _player, std::vector<Mob*>& _mob, std::vector<Building*>& _building, std::vector<Drop*>& _food, std::vector<Sort*>& _sort, std::vector<Scarecrow*>& _scarecrow, std::vector<Building*>& _sleepingNut)
{
	...
	Audio& _audio, std::vector<Building*>& _building, sf::Font& _font, Wave& _wave, std::vector<Scarecrow*>& _scarecrow, sf::Sprite& _spriteHeart)
{
	...
	for (int i = 0; i < _mob.size(); i++)
	{
		...
		{
			UpdateWitchHitBox(_mob[i]->GetWitch(), _player, _audio);
		}
	}
	...
	for (int i = 0; i < _scarecrow.size(); i++)
	{
		MobArrowCollision(_projectile, _scarecrow, _audio);
...
void EntityManager::UpdateVector(float _dt, std::vector<Player*>& _player, std::vector<Mob*>& _mob, std::vector<Scarecrow*>& _scarecrow, std::vector<Projectile>& _projectile, std::vector<Building*>& _building, std::vector<Drop*>& _food, std::vector<Sort*>& _sort, sf::Font& _font, sf::Sprite& _spriteHeart, std::vector<Building*>& _sleepingNut)
{
	...
	FillVectorSort(_projectile, _player, _mob, _building, _food, _sort, _scarecrow, _sleepingNut);
}
