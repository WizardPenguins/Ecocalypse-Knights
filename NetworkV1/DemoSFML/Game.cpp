...
void Game::InitClasses(GameData& _gameData)
{
	...
	_gameData.rtRed->create((unsigned int)(Screen::GetInstance().GetWidth() * 7.f), (unsigned int)(Screen::GetInstance().GetHeight() * 4.5f));

	_gameData.rtBlue = new sf::RenderTexture();
	_gameData.rtBlue->create((unsigned int)(Screen::GetInstance().GetWidth() * 7.f), (unsigned int)(Screen::GetInstance().GetHeight() * 4.5f));
	...
	if (_gameData.levelState == LEVEL1)
	{
		...
		{
			PopUp* dialogue = new PopUp(i);
			_gameData.popUpVector.push_back(dialogue);
		}
	}
}
...
void Game::Update(GameData& _gameData)
{
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::K))
	{
		...
		{
			_gameData.tabMob[i]->SetLife(0);
			_gameData.tabMob[i]->SetState(MobState::Death);
		}
	}

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::F))
	{
		_gameData.tabPlayers[0]->SetFreeFall(true);
		_gameData.tabPlayers[0]->SetPlayerState(FREEFALL);
	}
}
...
void Game::Level1Handler(GameData& _gameData)
{
	...
	UpdatePopUp(_gameData.currentPopUp, _gameData.popUpVector, _gameData.TutoActive, _gameData.dt);
	...
	//update Sorting
	_gameData.entityMan->UpdateVector(_gameData.dt, _gameData.tabPlayers, _gameData.tabMob, _gameData.scarecrow, _gameData.projectile, _gameData.tabBuilding, _gameData.drop, _gameData.sortVector, *_gameData.GameFont, _gameData.heartHit, _gameData.sleepingNut);
...
void Game::UpdateMultiplayer(GameData& _gameData)
{
	...
	{
		_gameData.network->IngameStateUpdateGame(_gameData, &_gameData.levelState);
...
void Game::UpdateDeath(GameData& _gameData)
{
	...
	else
	{
		for (int i = 0; i < _gameData.tabMob.size(); i++)
		{
			if (death == _gameData.tabPlayers.size() || _gameData.tabMob[i]->GetType() == TypeMob::Boss && _gameData.tabMob[i]->GetState() == MobState::Death)
			{
				timerDeath += _gameData.dt;
				if (timerDeath >= 2.f * _gameData.tabMob.size())
				{
					_gameData.reloadGameOver = true;
					FadeManagement(_gameData);
				}
...
void Game::Level1Display(GameData& _gameData)
{
	...
	DisplayPopUp(_gameData.currentPopUp, _gameData.popUpVector, _gameData.TutoActive, _gameData.window);

	DisplayHand(_gameData.tabMob, _gameData.window);
...
void Game::LevelDisplay3D(GameData& _gameData)
{
	...
	_gameData.camera->DisplayFirstPlan(_gameData.renderTextureRed, _gameData.renderTextureBlue, _gameData.levelState);
	...
	DisplayPopUp3D(_gameData.currentPopUp, _gameData.popUpVector, _gameData.TutoActive, _gameData.renderTextureRed, _gameData.renderTextureBlue);

	DisplayHand(_gameData.tabMob, _gameData.renderTextureRed, _gameData.renderTextureBlue);
...
void Game::Level3Display(GameData& _gameData)
{
	...
	if (!_gameData.tabPlayers[0]->playerIsShrinks)
	{
		_gameData.window.draw(_gameData.rtShadow);
	}
	//Viper Spell
...
void Game::DisplayHand(std::vector<Mob*>& _mob, sf::RenderWindow& _window)
{
	if (!_mob.size())//empty
	{
	...
	else
	{
		for (int i = 0; i < _mob.size(); i++)
		{
			if (_mob[i]->GetType() == TypeMob::Boss && _mob[i]->GetState() == MobState::Death)
			{
				Utils::BlitSprite(nextLevel, { (float)(currentPos.x), (float)(currentPos.y) }, _window);
			}
		}
...
void Game::DisplayHand(std::vector<Mob*>& _mob, sf::RenderTexture& _red, sf::RenderTexture& _blue)
{
	if (!_mob.size())//empty
	{
...
void Game::MapHandler(GameData& _gameData)
{
	...
	if (sf::Joystick::getIdentification(0).name == "Wireless Controller" && _gameData.timerButton <= 0.f)
	{
