...
void CharacterSelection::InitComponents(GameData& _gameData)
{
	...
	_gameData.network->data.tabSkin = &this->playerSkins;
}
...
void CharacterSelection::UpdateEasing(int i, float _dt, bool _moveMenuToPlayerSelection, Audio& _audio)
{
	...
	if (playerSkins[i].slidingUp) // stick up
	{
		...
		playerSkins[i].timerSpriteSkinPlayer += _dt;
...
void CharacterSelection::FadeGoToGame(bool _moveMenuToPlayerSelection, int _i, float _dt, bool _teamfight, bool _arena)
{
...
void CharacterSelection::DisplayWarningController(sf::RenderTexture& _red, sf::RenderTexture& _blue)
{
	if (!sf::Joystick::isConnected(0) && !sf::Joystick::isConnected(1) && !sf::Joystick::isConnected(2) && !sf::Joystick::isConnected(3))
	{		
		Utils::BlitSprite(manetteEncule, sf::Vector2f{ (float)((WORKING_WIDTH - 503) / 2) + (TROAD_GAP * (-1 * (float)(WORKING_HEIGHT / 2 + HEIGHT_GAP - manetteEncule.getGlobalBounds().height / 2) + WORKING_HEIGHT) / 100.f),(float)(WORKING_HEIGHT / 2 + HEIGHT_GAP - manetteEncule.getGlobalBounds().height / 2) }, 0, _red);
		Utils::BlitSprite(manetteEncule, sf::Vector2f{ (float)((WORKING_WIDTH - 503) / 2) - (TROAD_GAP * (-1 * (float)(WORKING_HEIGHT / 2 + HEIGHT_GAP - manetteEncule.getGlobalBounds().height / 2) + WORKING_HEIGHT) / 100.f),(float)(WORKING_HEIGHT / 2 + HEIGHT_GAP - manetteEncule.getGlobalBounds().height / 2) }, 0, _blue);
	}
}
...
void CharacterSelection::Display(GameData& _gameData)
{
	...
	for (int i = 0; i < _gameData.tabPlayers.size(); i++)
	{
		FadeGoToGame(_gameData.camera->moveMenuToPlayerSelection, i, _gameData.dt, _gameData.teamfight, _gameData.arenaIsChose);
	...
	{
		_gameData.window.setView(_gameData.camera->GetView());
		_gameData.network->spriteIPBar.scale({ 0.5, 0.5 });
		_gameData.network->spriteIPBar.setPosition({ WORKING_WIDTH / 2 - WIDTH_IP_BAR / 4, WORKING_HEIGHT});
		_gameData.window.draw(_gameData.network->spriteIPBar);
		if (_gameData.network->isRevealed)
		{
			_gameData.network->tabText[0]->setString(sf::IpAddress::getLocalAddress().toString());
			_gameData.network->tabText[0]->setCharacterSize(60);
			_gameData.network->tabText[0]->setPosition({WORKING_WIDTH / 2 - 100, WORKING_HEIGHT + 10});
...
void CharacterSelection::Display3D(GameData& _gameData)
{
	...
	for (int i = 0; i < _gameData.tabPlayers.size(); i++)
	{
		...
		//display background of the frame
		Utils::BlitSprite(playerSkins[i].spriteBackgroundFrame, sf::Vector2f{ playerSkins[i].spriteBackgroundFrame.getPosition().x + (TROAD_GAP * (-1 * playerSkins[i].spriteBackgroundFrame.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteBackgroundFrame.getPosition().y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(playerSkins[i].spriteBackgroundFrame, sf::Vector2f{ playerSkins[i].spriteBackgroundFrame.getPosition().x - (TROAD_GAP * (-1 * playerSkins[i].spriteBackgroundFrame.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteBackgroundFrame.getPosition().y }, 0, _gameData.renderTextureBlue);

		//display skins
		if (!_gameData.camera->moveMenuToPlayerSelection)
		{
			Utils::BlitSprite(playerSkins[i].nextSkinSelected, sf::Vector2f{ playerSkins[i].posNextSkinSelected.x + (TROAD_GAP * (-1 * playerSkins[i].posNextSkinSelected.y + WORKING_HEIGHT) / 100.f),playerSkins[i].posNextSkinSelected.y }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(playerSkins[i].nextSkinSelected, sf::Vector2f{ playerSkins[i].posNextSkinSelected.x - (TROAD_GAP * (-1 * playerSkins[i].posNextSkinSelected.y + WORKING_HEIGHT) / 100.f),playerSkins[i].posNextSkinSelected.y }, 0, _gameData.renderTextureBlue);
		}

		Utils::BlitSprite(playerSkins[i].spriteSkinPlayer, sf::Vector2f{ playerSkins[i].posSpriteSkinPlayer.x + (TROAD_GAP * (-1 * playerSkins[i].posSpriteSkinPlayer.y + WORKING_HEIGHT) / 100.f),playerSkins[i].posSpriteSkinPlayer.y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(playerSkins[i].spriteSkinPlayer, sf::Vector2f{ playerSkins[i].posSpriteSkinPlayer.x - (TROAD_GAP * (-1 * playerSkins[i].posSpriteSkinPlayer.y + WORKING_HEIGHT) / 100.f),playerSkins[i].posSpriteSkinPlayer.y }, 0, _gameData.renderTextureBlue);
	}

	Utils::BlitSprite(spriteMask, sf::Vector2f{ spriteMask.getPosition().x - (TROAD_GAP * (-1 * spriteMask.getPosition().y + WORKING_HEIGHT) / 100.f),spriteMask.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(spriteMask, sf::Vector2f{ spriteMask.getPosition().x + (TROAD_GAP * (-1 * spriteMask.getPosition().y + WORKING_HEIGHT) / 100.f),spriteMask.getPosition().y }, 0, _gameData.renderTextureBlue);

	//Display encadré des skins
	for (int i = 0; i < _gameData.tabPlayers.size(); i++)
	{
		Utils::BlitSprite(playerSkins[i].spriteFramePlayer, sf::Vector2f{ playerSkins[i].spriteFramePlayer.getPosition().x + (TROAD_GAP * (-1 * playerSkins[i].spriteFramePlayer.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteFramePlayer.getPosition().y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(playerSkins[i].spriteFramePlayer, sf::Vector2f{ playerSkins[i].spriteFramePlayer.getPosition().x - (TROAD_GAP * (-1 * playerSkins[i].spriteFramePlayer.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteFramePlayer.getPosition().y }, 0, _gameData.renderTextureBlue);

		Utils::BlitSprite(playerSkins[i].spriteNumberPlayerCurrent, sf::Vector2f{ playerSkins[i].spriteNumberPlayerCurrent.getPosition().x + (TROAD_GAP * (-1 * playerSkins[i].spriteNumberPlayerCurrent.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteNumberPlayerCurrent.getPosition().y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(playerSkins[i].spriteNumberPlayerCurrent, sf::Vector2f{ playerSkins[i].spriteNumberPlayerCurrent.getPosition().x - (TROAD_GAP * (-1 * playerSkins[i].spriteNumberPlayerCurrent.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteNumberPlayerCurrent.getPosition().y }, 0, _gameData.renderTextureBlue);

		Utils::BlitSprite(playerSkins[i].spriteLockSkin, sf::Vector2f{ playerSkins[i].spriteLockSkin.getPosition().x + (TROAD_GAP * (-1 * playerSkins[i].spriteLockSkin.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteLockSkin.getPosition().y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(playerSkins[i].spriteLockSkin, sf::Vector2f{ playerSkins[i].spriteLockSkin.getPosition().x - (TROAD_GAP * (-1 * playerSkins[i].spriteLockSkin.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteLockSkin.getPosition().y }, 0, _gameData.renderTextureBlue);

		if (_gameData.teamfight)
		{
			Utils::BlitSprite(playerSkins[i].spriteTeamChoice, sf::Vector2f{ playerSkins[i].spriteTeamChoice.getPosition().x + (TROAD_GAP * (-1 * playerSkins[i].spriteTeamChoice.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteTeamChoice.getPosition().y }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(playerSkins[i].spriteTeamChoice, sf::Vector2f{ playerSkins[i].spriteTeamChoice.getPosition().x - (TROAD_GAP * (-1 * playerSkins[i].spriteTeamChoice.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteTeamChoice.getPosition().y }, 0, _gameData.renderTextureBlue);

			Utils::BlitSprite(playerSkins[i].spriteLockTeam, sf::Vector2f{ playerSkins[i].spriteLockTeam.getPosition().x + (TROAD_GAP * (-1 * playerSkins[i].spriteLockTeam.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteLockTeam.getPosition().y }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(playerSkins[i].spriteLockTeam, sf::Vector2f{ playerSkins[i].spriteLockTeam.getPosition().x - (TROAD_GAP * (-1 * playerSkins[i].spriteLockTeam.getPosition().y + WORKING_HEIGHT) / 100.f),playerSkins[i].spriteLockTeam.getPosition().y }, 0, _gameData.renderTextureBlue);
		}
	}

	DisplayWarningController(_gameData.renderTextureRed, _gameData.renderTextureBlue);

	Utils::BlitSprite(shadowFountain, sf::Vector2f{ shadowFountain.getPosition().x + (TROAD_GAP * (-1 * shadowFountain.getPosition().y + WORKING_HEIGHT) / 100.f),shadowFountain.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(shadowFountain, sf::Vector2f{ shadowFountain.getPosition().x - (TROAD_GAP * (-1 * shadowFountain.getPosition().y + WORKING_HEIGHT) / 100.f),shadowFountain.getPosition().y }, 0, _gameData.renderTextureBlue);
	...
	for (int i = 0; i < _nbPlayers; i++)
	{
		if (sf::Joystick::getIdentification(GetFirstController(i)).name == "Wireless Controller")
		{
			if (sf::Joystick::isButtonPressed(GetFirstController(i), (unsigned int)ControllerButtonPS4::CROSS))
			{
				...
				else if (sf::Joystick::isButtonPressed(GetFirstController(0), (unsigned int)ControllerButtonPS4::CROSS) && playerSkins[i].isReady && !roundNumberIsSelected && _arena && !_teamfight)
				{
			...
			{
				if (playerSkins[i].isReady && _camera.moveMenuToPlayerSelection == false && !playerSkins[i].teamSelected)
				{
					_audio.PlaySoundNotRepeated("BUTTON_CLICK_EFFECT", 100.f);
					playerSkins[i].isReady = false;
					playerSkins[i].spriteLockSkin = allPlayerLockSkins[2];
