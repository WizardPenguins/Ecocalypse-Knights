...
Boar::Boar(Mob* _mob, BoarSprite* _sprite)
{
	...
	this->timerBeforeAttack = timerBeforeAttack;
	...
	this->sprite.animStun = new Animation;
	InitNewAnim();
	CreateAnimStun();
...
void Boar::Update(std::vector<Player*>& _tabPlayer, float _dt, Wave _wave, Audio& _audio)
{
	...
	if (this->timerBeforeAttack <= 0)
	{
		...
		if (groinkTimerSound > 3.5f && this->state != BoarState::Death)
		{
...
void Boar::DisplayEntity(sf::RenderWindow& _window)
{
	...
	if (this->state == BoarState::KnockDown)
	{
		Utils::BlitSprite(this->sprite.spriteStun.sprite, sf::Vector2f{this->mob->pos.x,this->mob->pos.y - 250.f}, 0, _window);
	}
}
...
void Boar::DisplayEntity(sf::RenderTarget& _red, sf::RenderTarget& _blue)
{
	...
	if (this->state == BoarState::KnockDown)
	{
		Utils::BlitSprite(this->sprite.spriteStun.sprite, sf::Vector2f{ this->mob->pos.x + (TROAD_GAP * (-1 * (this->mob->pos.y - 150.f) + WORKING_HEIGHT) / 100.f),(this->mob->pos.y - 150.f) }, 0, _red);
		Utils::BlitSprite(this->sprite.spriteStun.sprite, sf::Vector2f{ this->mob->pos.x - (TROAD_GAP * (-1 * (this->mob->pos.y - 150.f) + WORKING_HEIGHT) / 100.f),(this->mob->pos.y - 150.f) }, 0, _blue);
	}
}
...
void Boar::CreateAnimStun()
{
	sf::IntRect rect = sf::IntRect(0, 0, (int)(this->sprite.spriteStun.animDim.x / this->sprite.spriteStun.nbAnim.x), (int)(this->sprite.spriteStun.animDim.y));
	this->sprite.animStun->CreateLeftToRight(this->sprite.spriteStun.sprite, (const unsigned char)(this->sprite.spriteStun.nbAnim.x), (const unsigned char)(this->sprite.spriteStun.nbAnim.y), this->sprite.spriteStun.animDim);
	this->sprite.spriteStun.sprite.setTextureRect(rect);
	this->sprite.animStun->currentFrame = 0;
}
...
void Boar::UpdateZenitsuAttack(sf::Vector2f _player, float _dt, Audio& _audio)
{
	...
	if (this->sprite.anim->currentFrame == this->sprite.anim->maxFrame && !this->dirAnim) //end frame of animation
	{
		this->dirAnim = 1;
		this->mob->pos = sf::Vector2f{ _player.x + 200.f,_player.y };
		this->sprite.anim->currentFrame = 0;
	}
	else if (this->sprite.anim->currentFrame == this->sprite.anim->maxFrame && this->dirAnim)
	{
	...
	else if (this->sprite.anim->currentFrame >= 1 && this->sprite.anim->currentFrame <= 13 && this->dirAnim) //frame when attack
	{
		this->mob->pos = sf::Vector2f{ _player.x + 200.f,_player.y };
	}
...
void Boar::UpdateAnim(float _dt)
{
	...
	{
		bool temp = true;
		this->sprite.animStun->UpdateLeftToRight(this->sprite.spriteStun.sprite, _dt, 14, 0, temp, this->sprite.spriteStun.loop);
	}
