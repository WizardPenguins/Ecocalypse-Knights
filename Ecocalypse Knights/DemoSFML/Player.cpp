...
void Player::Update(GameData& _gameData, float _dtSlowMo, bool _dontMoveArena)
{
	if (isLoading)
	{
		...
		if (_gameData.bridgeStartBroken)
		{
			if (state != IDLE)
			{
				SetPlayerState(IDLE);
		...
		UpdateInventory(_dtSlowMo, *_gameData.audio);
...
void Player::Display(Audio& _audio, sf::RenderTexture& _textureRed, sf::RenderTexture& _textureBlue, Camera* _camera, LevelState& _levelState)
{
	...
	else
	{
		...
		potionEffect.setScale(SHRINKS, SHRINKS);
	...
	sf::Vector2f posPlayer = this->sprite.getPosition();

	(bool)(this->inAir) ? (bool)(this->heavyAttackGoUp) ? this->sprite.setPosition(posPlayer.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posPlayer.y) : this->sprite.setPosition({ posPlayer.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),posPlayer.y }) : this->sprite.setPosition({ posPlayer.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),posPlayer.y });
	if (this->state == KNOCKDOWN)
	{
		this->sprite.setPosition(posPlayer.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posPlayer.y);
	}
	...
	this->sprite.setPosition(posPlayer);
...
void Player::Display(sf::RenderWindow& _window, Audio& _audio, Camera* _camera, LevelState& _levelState)
{
	...
	if (showPotionAnime)
	{
		...
		{
			Utils::BlitSprite(potionEffect, { pos.x,posInAir.y }, 0, _window);
		}
	}
...
void Player::UpdatePlayerMapColliding(GameData& _gameData)
{
	...
	else if (_gameData.levelState == LEVEL3)
	{
		...
		if ((freefallWave && state == LIGHTATTACK) || (freefallWave && state == HIT) || state == FREEFALL)
		{
			collideWithYellow = false;
		}
		else
		{
			collideWithYellow = true;
		}
		
		if (_gameData.tempImage3.getPixel(this->pos.x, this->pos.y) == sf::Color(255, 255, 0) && collideWithYellow)
		{
			this->pos = lastPos;
		}
...
void Player::UpdateCinematic(GameData& _gameData)
{
	if (_gameData.levelState == LEVEL3)
	{
		...
		if (freefallStartCinematic)
		{
			...
			{
				pos = Easings::Lerp(startPosEasingfreefallCinematic, endPosEasingfreefallCinematic, timerEasingfreefallCinematic);
				timerEasingfreefallCinematic += _gameData.dt/4.8f;
			}
			else
			{
				freefallStartCinematic = false;
				timerEasingfreefallCinematic = 0.f;
			}
		}
		
		if (!freefallStartCinematic && freefallWave && _gameData.tabMob.size() == 0)
		{
			SetFreeFall(false);
		}
		...
		if (freefallEndCinematic)
		{
			...
			{
				pos = Easings::Lerp(startPosEasingfreefallCinematic, endPosEasingfreefallCinematic, timerEasingfreefallCinematic);
				timerEasingfreefallCinematic += _gameData.dt/1.3f;
			}
			else
			{
				freefallEndCinematic = false;
				timerEasingfreefallCinematic = 0.f;
				if (state != DEAD)
				{
					SetPlayerState(HIT_GROUND);
				}
			}
		}
	}
...
void Player::UpdateInventory(float _dt, Audio& _audio)
{
	if (useItem)
	{
		if (hasAPotion && life > 0)
		{
			_audio.PlaySoundNotRepeated("PLAYER_GLOUGLOU");
...
void Player::LevelUp(Audio& _audio)
{
	while (this->xp >= this->maxXp)
	{
		_audio.PlaySoundNotRepeated("PLAYER_LEVELUP");
		this->level++;
		this->skillPoints += 2;
		this->xp -= this->maxXp;
...
{
	if (freefallWave || freefallStartCinematic || freefallEndCinematic)
	{
		return true;
	}
	else
...
void Player::SetFreeFall(bool _bool)
{
	...
	if (this->inAir)
	{
		this->pos = this->posInAir;
		this->inAir = false;
	}

	if (freefallStartCinematic)
	{
		SetPlayerState(FREEFALL);
	}

	this->startPosEasingfreefallCinematic = this->pos;
	(bool)(this->freefallEndCinematic) ? this->endPosEasingfreefallCinematic = { this->pos.x, 4064 } : this->endPosEasingfreefallCinematic = { this->pos.x, 3064 };

	if (this->pos.x < 10000 && this->freefallEndCinematic)
	{
		this->endPosEasingfreefallCinematic.x = 10000;
	}
	else if (this->pos.x > 11300 && this->freefallEndCinematic)
	{
		this->endPosEasingfreefallCinematic.x = 11300;
	}
}

void Player::ResetPotionEffectBetweenLevel(void)
{
	this->showPotionAnime = false;
	if (hasUseSpeedBuff)
	{
		hasUseSpeedBuff = false;
		stats[(int)Stats::AGILITY] -= 3;
	...
	else if (hasUseStrengthBuff)
	{
		hasUseStrengthBuff = false;
		stats[(int)Stats::STRENGTH] -= 3;
		timerItemUse = 0.f;
		}
	else if (hasUseMagicBuff)
	{
		hasUseMagicBuff = false;
		stats[(int)Stats::WISDOM] -= 3;
		timerItemUse = 0.f;
...
bool Player::GetHasAPotion(void)
{
	if (hasAPotion || hasAMagicBuff || hasASpeedBuff || hasAStrengthBuff)
	{
		return true;
	}
	else
...
bool Player::GetFreefallEndCinematicBird(void)
{
	return !this->freefallStartCinematic;
}

bool Player::GetFreefallStartCinematicBoss(void)
{
	return this->freefallEndCinematic;
}

bool Player::GetAsDoAFreeFall(void)
{
	return asDoAFreeFall;
}
