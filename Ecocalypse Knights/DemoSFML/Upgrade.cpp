...
void Upgrade::DownUpJoysticksButtonsUpgrade(GameData& _gameData)
{
	if (!buttonVectUpgrade.empty())
	{
		...
		if ((yPos1 < 15 && yPos1 > -15) && (xPos1 < 15 && xPos1 > -15))
		{
			firstMoveJoystick = true;
		}

		if ((yPos2 < 15 && yPos2 > -15) && (xPos2 < 15 && xPos2 > -15))
		{
			firstMoveCross = true;
		}

		if (joystickTimer >= 0.2f || firstMoveJoystick)
		{
			if (yPos1 > 70)
			{
				joystickTimer = 0;
				firstMoveJoystick = false;
			...
			//add
			if ((xPos1 > 20) && _gameData.tabPlayers[currentPlayer]->GetSkillPoints())
			{
				joystickTimer = 0;
				firstMoveJoystick = false;
					...
					if (buttonVectUpgrade[i]->GetIsSelected())
					{
						...
						case 0:
							if (_gameData.tabPlayers[currentPlayer]->GetVigor() != MaxStat)
							{
								...
								_gameData.audio->PlaySoundNotRepeated("UPGRADE_PLUS");
							}
							break;
						case 1:
					...
					if (buttonVectUpgrade[i]->GetIsSelected())
					{
						...
						case 0:
							if (_gameData.tabPlayers[currentPlayer]->GetVigor() != BaseStat && !this->filledBLock[this->currentColorPlayer][0][_gameData.tabPlayers[currentPlayer]->GetVigor() - 1].lock)
							{
								...
								_gameData.audio->PlaySoundNotRepeated("UPGRADE_MINUS");
							}
							break;
						case 1:
		...
		if (joystickTimer >= 0.2f || firstMoveCross)
		{
			if (yPos2 < -50)
			{
				joystickTimer = 0;
				firstMoveCross = false;
				if (buttonSelectedUpgrade != (buttonVectUpgrade.size() - 1))
				{
					++buttonSelectedUpgrade;
				}
			...
			//add
			if ((xPos2 > 20) && _gameData.tabPlayers[currentPlayer]->GetSkillPoints())
			{
				joystickTimer = 0;
				firstMoveCross = false;
				for (int i = 0; i < buttonVectUpgrade.size(); i++)
...
void Upgrade::Update(GameData& _gameData)
{
	if (!this->init)
	{
		...
		while (!_gameData.tabPlayers[this->currentPlayer]->GetSkillPoints())
		{
			...
			this->currentPlayer++;
			this->currentColorPlayer = _gameData.tabPlayers[currentPlayer]->GetPlayerSelected();
		}
	}
...
void Upgrade::Display(GameData& _gameData)
{
	...
	if (this->buttonSelectedUpgrade <= 3)
	{
		...
		if (this->buttonSelectedUpgrade == 2 && this->filledBLock[this->currentColorPlayer][2][4].colored)
		{
			this->nbDash++;
			this->textUnlockSkill.setString("+" + std::to_string(this->nbDash) + " Dash");
		}
		if (this->buttonSelectedUpgrade == 2 && this->filledBLock[this->currentColorPlayer][2][9].colored)
		{
			this->nbDash++;
			this->textUnlockSkill.setString("+" + std::to_string(this->nbDash) + " Dash");
		}
...
{
	Utils::BlitSprite(this->backGround, sf::Vector2f{ this->backGround.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),this->backGround.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(this->backGround, sf::Vector2f{ this->backGround.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),this->backGround.getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(this->confirm[0]->sprite, sf::Vector2f{ this->confirm[0]->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),this->confirm[0]->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(this->confirm[0]->sprite, sf::Vector2f{ this->confirm[0]->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),this->confirm[0]->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);

	for (int i = 0; i < buttonVectUpgrade.size(); i++)
	{
		Utils::BlitSprite(this->plank[i][1]->sprite, sf::Vector2f{ this->plank[i][1]->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->plank[i][1]->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(this->plank[i][1]->sprite, sf::Vector2f{ this->plank[i][1]->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->plank[i][1]->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);

		if (buttonVectUpgrade[i]->GetIsSelected())
		{
			if (i <= 3)
			{
				Utils::BlitSprite(this->plank[buttonSelectedUpgrade][0]->sprite, sf::Vector2f{ this->plank[buttonSelectedUpgrade][0]->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->plank[buttonSelectedUpgrade][0]->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
				Utils::BlitSprite(this->plank[buttonSelectedUpgrade][0]->sprite, sf::Vector2f{ this->plank[buttonSelectedUpgrade][0]->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->plank[buttonSelectedUpgrade][0]->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);
			}
			else
			{
				Utils::BlitSprite(this->confirm[2]->sprite, sf::Vector2f{ this->confirm[2]->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->confirm[2]->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
				Utils::BlitSprite(this->confirm[2]->sprite, sf::Vector2f{ this->confirm[2]->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->confirm[2]->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);
			}
		}
	}
	...
	for (int i = 0; i < STATS_NBR; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			if (buttonSelectedUpgrade == i)
			{
				Utils::BlitSprite(this->selectButton[currentColorPlayer][buttonSelectedUpgrade][j]->sprite, sf::Vector2f{ this->selectButton[currentColorPlayer][buttonSelectedUpgrade][j]->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->selectButton[currentColorPlayer][buttonSelectedUpgrade][j]->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
				Utils::BlitSprite(this->selectButton[currentColorPlayer][buttonSelectedUpgrade][j]->sprite, sf::Vector2f{ this->selectButton[currentColorPlayer][buttonSelectedUpgrade][j]->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->selectButton[currentColorPlayer][buttonSelectedUpgrade][j]->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);
			}
			else
			{
				Utils::BlitSprite(this->justButton[currentColorPlayer][i][j]->sprite, sf::Vector2f{ this->justButton[currentColorPlayer][i][j]->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->justButton[currentColorPlayer][i][j]->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
				Utils::BlitSprite(this->justButton[currentColorPlayer][i][j]->sprite, sf::Vector2f{ this->justButton[currentColorPlayer][i][j]->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->justButton[currentColorPlayer][i][j]->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);
			}
		}
	...
	for (int j = 0; j < NbMaxPlanks; j++)
	{
		for (int k = 0; k < NbMaxPoint; k++)
		{
			Utils::BlitSprite(this->filledBLock[this->currentColorPlayer][j][k].sprite, sf::Vector2f{ this->filledBLock[this->currentPlayer][j][k].pos.x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->filledBLock[this->currentPlayer][j][k].pos.y }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(this->filledBLock[this->currentColorPlayer][j][k].sprite, sf::Vector2f{ this->filledBLock[this->currentPlayer][j][k].pos.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->filledBLock[this->currentPlayer][j][k].pos.y }, 0, _gameData.renderTextureBlue);
			this->filledBLock[this->currentColorPlayer][j][k].sprite.setScale(1.f, 1.f);
		}
	}

	Utils::BlitSprite(this->colorBackGround[currentColorPlayer]->sprite, sf::Vector2f{ this->colorBackGround[currentColorPlayer]->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->colorBackGround[currentColorPlayer]->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(this->colorBackGround[currentColorPlayer]->sprite, sf::Vector2f{ this->colorBackGround[currentColorPlayer]->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->colorBackGround[currentColorPlayer]->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(this->characterFrame[currentColorPlayer]->sprite, sf::Vector2f{ this->characterFrame[currentColorPlayer]->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->characterFrame[currentColorPlayer]->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(this->characterFrame[currentColorPlayer]->sprite, sf::Vector2f{ this->characterFrame[currentColorPlayer]->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->characterFrame[currentColorPlayer]->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(this->pointsPlank->sprite, sf::Vector2f{ this->pointsPlank->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->pointsPlank->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(this->pointsPlank->sprite, sf::Vector2f{ this->pointsPlank->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->pointsPlank->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(this->textPlank->sprite, sf::Vector2f{ this->textPlank->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->textPlank->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(this->textPlank->sprite, sf::Vector2f{ this->textPlank->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->textPlank->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(this->mult->sprite, sf::Vector2f{ this->mult->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->mult->sprite.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(this->mult->sprite, sf::Vector2f{ this->mult->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->mult->sprite.getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(this->filledBlocksPlank[currentColorPlayer], sf::Vector2f{ this->filledBlocksPlank[currentColorPlayer].getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->filledBlocksPlank[currentColorPlayer].getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(this->filledBlocksPlank[currentColorPlayer], sf::Vector2f{ this->filledBlocksPlank[currentColorPlayer].getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->filledBlocksPlank[currentColorPlayer].getPosition().y }, 0, _gameData.renderTextureBlue);



	sf::Vector2f posPoint = this->skillPoints.getPosition();

	this->skillPoints.setPosition(sf::Vector2f{ posPoint.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posPoint.y });
	_gameData.renderTextureRed.draw(this->skillPoints);

	this->skillPoints.setPosition(sf::Vector2f{ posPoint.x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posPoint.y });
	_gameData.renderTextureBlue.draw(this->skillPoints);

	this->skillPoints.setPosition(sf::Vector2f{ posPoint });



	//anim player
	Utils::BlitSprite(this->idlePlayer[this->currentColorPlayer], sf::Vector2f{ this->idlePlayer[this->currentColorPlayer].getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->idlePlayer[this->currentColorPlayer].getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(this->idlePlayer[this->currentColorPlayer], sf::Vector2f{ this->idlePlayer[this->currentColorPlayer].getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->idlePlayer[this->currentColorPlayer].getPosition().y }, 0, _gameData.renderTextureBlue);


	//upgrade stat
	if (this->buttonSelectedUpgrade <= 3)
	{
		Utils::BlitSprite(stats[this->buttonSelectedUpgrade], sf::Vector2f{ 90.f - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), 600.f }, -13, _gameData.renderTextureRed);
		Utils::BlitSprite(stats[this->buttonSelectedUpgrade], sf::Vector2f{ 90.f + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), 600.f }, -13, _gameData.renderTextureBlue);

		sf::Vector2f posBefore = this->maxBefore.getPosition();
		sf::Vector2f posAfter = this->maxAfter.getPosition();
		sf::Vector2f posStat = this->textStat.getPosition();



		this->maxBefore.setPosition(sf::Vector2f{ posBefore.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posBefore.y });
		_gameData.renderTextureRed.draw(this->maxBefore);

		this->maxBefore.setPosition(sf::Vector2f{ posBefore.x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posBefore.y });
		_gameData.renderTextureBlue.draw(this->maxBefore);

		this->maxBefore.setPosition(sf::Vector2f{ posBefore });



		this->maxAfter.setPosition(sf::Vector2f{ posAfter.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posAfter.y });
		_gameData.renderTextureRed.draw(this->maxAfter);

		this->maxAfter.setPosition(sf::Vector2f{ posAfter.x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posAfter.y });
		_gameData.renderTextureBlue.draw(this->maxAfter);

		this->maxAfter.setPosition(sf::Vector2f{ posAfter });



		this->textStat.setPosition(sf::Vector2f{ posStat.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posStat.y });
		_gameData.renderTextureRed.draw(this->textStat);

		this->textStat.setPosition(sf::Vector2f{ posStat.x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posStat.y });
		_gameData.renderTextureBlue.draw(this->textStat);

		this->textStat.setPosition(sf::Vector2f{ posStat });



		Utils::BlitSprite(this->arrow, sf::Vector2f{ this->arrow.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->arrow.getPosition().y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(this->arrow, sf::Vector2f{ this->arrow.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->arrow.getPosition().y }, 0, _gameData.renderTextureBlue);

		if ((this->buttonSelectedUpgrade == 2 && (this->filledBLock[this->currentColorPlayer][2][4].colored || this->filledBLock[this->currentColorPlayer][2][9].colored)) ||
			(this->buttonSelectedUpgrade == 3 && this->filledBLock[this->currentColorPlayer][3][4].colored))
		{
			sf::Vector2f posUnlock = this->textUnlockSkill.getPosition();

			this->textUnlockSkill.setPosition(sf::Vector2f{ posUnlock.x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posUnlock.y });
			_gameData.renderTextureRed.draw(this->textUnlockSkill);

			this->textUnlockSkill.setPosition(sf::Vector2f{ posUnlock.x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), posUnlock.y });
			_gameData.renderTextureBlue.draw(this->textUnlockSkill);

			this->textUnlockSkill.setPosition(sf::Vector2f{ posUnlock });
		}
	}

	for (int i = 0; i < 4; i++)
	{
		Utils::BlitSprite(this->stats[i], sf::Vector2f{ this->stats[i].getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->stats[i].getPosition().y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(this->stats[i], sf::Vector2f{ this->stats[i].getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), this->stats[i].getPosition().y }, 0, _gameData.renderTextureBlue);
	}
}
...
void Upgrade::InitReload(GameData& _gameData)
{
	InitUpgrade(_gameData);
	CrateAnimPlayerIdle();
	MaxStatPlayer(_gameData.tabPlayers[this->currentPlayer]);
