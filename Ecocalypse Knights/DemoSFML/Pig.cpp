...
void Pig::DisplayEntity(sf::RenderWindow& _window)
{
	...
	Utils::BlitSprite(this->sprite.spriteAnim[(int)this->state].sprite, sf::Vector2f{ this->sprite.spriteAnim[(int)this->state].sprite.getPosition().x, this->sprite.spriteAnim[(int)this->state].sprite.getPosition().y + DecalageSprite }, 0, _window);
}
...
void Pig::DisplayFx(sf::RenderTexture& _red, sf::RenderTexture& _blue)
{
	...
	{
		this->mob->inAir ? this->sprite.spriteFxHit.sprite.setPosition(sf::Vector2f{ this->mob->posInAir.x, this->mob->posInAir.y }) : this->sprite.spriteFxHit.sprite.setPosition(sf::Vector2f{ this->mob->pos.x, this->mob->pos.y });
		Utils::BlitSprite(this->sprite.spriteFxHit.sprite, sf::Vector2f{ this->sprite.spriteFxHit.sprite.getPosition().x - (TROAD_GAP * (-1 * this->sprite.spriteFxHit.sprite.getPosition().y + WORKING_HEIGHT) / 100.f), this->sprite.spriteFxHit.sprite.getPosition().y }, 0, _red);
		Utils::BlitSprite(this->sprite.spriteFxHit.sprite, sf::Vector2f{ this->sprite.spriteFxHit.sprite.getPosition().x + (TROAD_GAP * (-1 * this->sprite.spriteFxHit.sprite.getPosition().y + WORKING_HEIGHT) / 100.f), this->sprite.spriteFxHit.sprite.getPosition().y }, 0, _blue);
	}
}
...
{
	sf::IntRect rect = sf::IntRect(0, 0, (int)(this->sprite.spriteAnim[(int)this->state].animDim.x / this->sprite.spriteAnim[(int)this->state].nbAnim.x), (int)(this->sprite.spriteAnim[(int)this->state].animDim.y));
...
void Pig::Death(Audio& _audio)
{
	if (!this->mob->inAir && this->mob->life <= 0 && this->state != PigState::Death)
	{
		...
		this->mob->scaleSprite = { (float)this->mob->direction,1 };
...
{
	sf::Vector2f normVec = Utils::NormalizeVector(sf::Vector2f{ _posPlayer - this->mob->pos });
...
void Pig::UpdateDeath(float _dt)
{
	...
	{
		this->posDeath.x += DeathPosMoveX * this->mob->direction;
		this->mob->decalage.x += DeathPosMoveX * this->mob->direction;
	}
}
...
void Pig::Attack(float _dt)
{
	...
	if (this->timerSecu > 0)
	{
		sf::Vector2f goalPos = { this->mob->pos.x - (5000.f * this->mob->scaleSprite.x),this->mob->pos.y };
		MoveAttack(goalPos, _dt);
