...
PopUp::PopUp(int _number, sf::Sprite& _sprite)
{
	panel = _sprite;
...
void PopUp::Draw(sf::RenderTexture& _red, sf::RenderTexture& _blue)
{
	if (textIsSpawning || textIsDespawning)
	{
		Utils::BlitSprite(panel,sf::Vector2f{panel.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT)/ 100.f),panel.getPosition().y},0,_red);
		Utils::BlitSprite(panel,sf::Vector2f{panel.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT)/ 100.f),panel.getPosition().y},0,_blue);
	}
}