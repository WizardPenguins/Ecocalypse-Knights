...
{
	if (_gameData.reloadMapManager)
	{
		ReloadForGameOver(_gameData);
		_gameData.reloadMapManager = false;
	}

	PlayerChooseLevel(_gameData, _level2IsDone);
...
void MapManager::PlayerChooseLevel(GameData& _gameData, bool _level2IsDone)
{
	...
	for (int i = 0; i < NB_LEVEL; i++)
	{
		if (playerPos == posLevel[i] || playerPos.x >= posLevel[i].x - 1 && playerPos.x <= posLevel[i].x + 1 && playerPos.y >= posLevel[i].y - 1 && playerPos.y <= posLevel[i].y + 1)
		{
			...
			{
				if (playerIsOn == MAP2 && _level2IsDone)
				{
					timer = 0;
...
const char MapManager::SetLevel(GameData& _gameData)
{
	...
	for (int i = 0; i < NB_LEVEL; i++)
	{
		if (playerPos == posLevel[i] || playerPos.x >= posLevel[i].x - 1 && playerPos.x <= posLevel[i].x + 1 && playerPos.y >= posLevel[i].y - 1 && playerPos.y <= posLevel[i].y + 1)
		{
			...
			if (i + 1 == LEVELSHOP)
			{
				_gameData.audio->PlaySoundNotRepeated("MERCHANT_WELCOME1");
			}
			return i + 1;
		}
	}
...
void MapManager::UpdateCamera(GameData& _gameData)
{
	...
	{
		playerPos = { WORKING_WIDTH / 2, WORKING_HEIGHT / 2 };

	}
	else if (playerPos.x > WORKING_WIDTH * 1.5f && playerPos.y > WORKING_HEIGHT * 1.5f)
	{
		playerPos = { WORKING_WIDTH * 1.5f, WORKING_HEIGHT * 1.5f };

	}
	else if (playerPos.x > WORKING_WIDTH * 1.5f && playerPos.y < WORKING_HEIGHT / 2.f)
	{
		playerPos = { WORKING_WIDTH * 1.5f, WORKING_HEIGHT / 2.f };

	}
	else if (playerPos.x < WORKING_WIDTH / 2.f)
	{
		playerPos = { WORKING_WIDTH / 2.f, playerPos.y };

	}
	else if (playerPos.y < WORKING_HEIGHT / 2.f)
	{
		playerPos = { playerPos.x, WORKING_HEIGHT / 2.f };

	}
	else if (playerPos.x > WORKING_WIDTH * 1.5f)
	{
		playerPos = { WORKING_WIDTH * 1.5f, playerPos.y };

	}
	else if (playerPos.y > WORKING_HEIGHT * 1.5f)
	{
		playerPos = { playerPos.x, WORKING_HEIGHT * 1.5f };

	}
	view.setCenter(playerPos);
	/*float xPos1 = sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::RIGHT_JOYSTICK_X);
	float yPos1 = sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::RIGHT_JOYSTICK_Y);
	if (xPos1 > 15 || xPos1 < -15 || yPos1 > 15 || yPos1 < -15)
	{
		sf::Vector2f center = _gameData.camera->GetView().getCenter();
		if (xPos1 > 15)
		{
			center.x += xPos1 * 1000;
		}
		...
		_gameData.camera->GetView().setCenter(center);
	}
	else
	{
		_gameData.camera->GetView().setCenter(playerPos);
	}*/
}
...
void MapManager::Display3D(GameData& _gameData, bool _level2)
{
	...
	Utils::BlitSprite(backGroundDuBackGroundLevelSelector, sf::Vector2f{ backGroundDuBackGroundLevelSelector.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),backGroundDuBackGroundLevelSelector.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(backGroundDuBackGroundLevelSelector, sf::Vector2f{ backGroundDuBackGroundLevelSelector.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),backGroundDuBackGroundLevelSelector.getPosition().y }, 0, _gameData.renderTextureBlue);

	_gameData.renderTextureRed.setView(_gameData.camera->GetView());
	_gameData.renderTextureBlue.setView(_gameData.camera->GetView());

	Utils::BlitSprite(backGroundLevelSelector, sf::Vector2f{ 0 - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),0 }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(backGroundLevelSelector, sf::Vector2f{ 0 + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),0 }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(travelLevel2, sf::Vector2f{ 890.f + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), 1300.f }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(travelLevel2, sf::Vector2f{ 890.f - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), 1300.f }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(travelCaravan, sf::Vector2f{ 700.f + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),1360.f }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(travelCaravan, sf::Vector2f{ 700.f - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),1360.f }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(caravan, sf::Vector2f{ 600.f + (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f), 1400.f }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(caravan, sf::Vector2f{ 600.f - (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f), 1400.f }, 0, _gameData.renderTextureBlue);

	if (_level2)
	{
		Utils::BlitSprite(travelLevel3, sf::Vector2f{ 1240.f + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),1290.f }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(travelLevel3, sf::Vector2f{ 1240.f - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),1290.f }, 0, _gameData.renderTextureBlue);

		Utils::BlitSprite(level3, sf::Vector2f{ 2440.f + (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f),1300.f }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(level3, sf::Vector2f{ 2440.f - (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f),1300.f }, 0, _gameData.renderTextureBlue);

		if (level3Anim->currentFrame == level3Anim->maxFrame)
		{
			witch.setColor(sf::Color::Black);

			Utils::BlitSprite(witch, sf::Vector2f{ 1520.f + (TROAD_GAP * (-1 * 650.f + WORKING_HEIGHT) / 100.f),1270.f }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(witch, sf::Vector2f{ 1520.f - (TROAD_GAP * (-1 * 650.f + WORKING_HEIGHT) / 100.f),1270.f }, 0, _gameData.renderTextureBlue);
		}
	}

	Utils::BlitSprite(level2, sf::Vector2f{ 1090.f + (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f),1180.f }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(level2, sf::Vector2f{ 1090.f - (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f),1180.f }, 0, _gameData.renderTextureBlue);

	if (!_level2)
	{
		Utils::BlitSprite(locked, sf::Vector2f{ 1540.f + (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f),1350.f }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(locked, sf::Vector2f{ 1540.f - (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f),1350.f }, 0, _gameData.renderTextureBlue);

		if (level2Anim->currentFrame == level2Anim->maxFrame)
		{
			boar.setColor(sf::Color::Black);

			Utils::BlitSprite(boar, sf::Vector2f{ 1110.f + (TROAD_GAP * (-1 * 750.f + WORKING_HEIGHT) / 100.f),1190.f }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(boar, sf::Vector2f{ 1110.f - (TROAD_GAP * (-1 * 750.f + WORKING_HEIGHT) / 100.f),1190.f }, 0, _gameData.renderTextureBlue);
		}
	}
	...
	if (!playerIsOn)
	{
		if (_gameData.tabPlayers.size() == 1)
		{
			...
			Utils::BlitSprite(playerRun[_gameData.tabPlayers[0]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x + (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(playerRun[_gameData.tabPlayers[0]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x - (TROAD_GAP * (-1 * 700.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureBlue);
		}
		else
		{
			for (int i = _gameData.tabPlayers.size() - 1; i >= 0; i--)
			{
				...
				{
				case 0:
					Utils::BlitSprite(playerRun[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ (player.getPosition().x - 50.f) + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureRed);
					Utils::BlitSprite(playerRun[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ (player.getPosition().x - 50.f) - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureBlue);
					break;
				case 1:
					Utils::BlitSprite(playerRun[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ (player.getPosition().x + 50.f) + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureRed);
					Utils::BlitSprite(playerRun[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ (player.getPosition().x + 50.f) - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureBlue);
				...
				case 2:
					Utils::BlitSprite(playerRun[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x + (TROAD_GAP * (-1 * (800.f - 50.f) + WORKING_HEIGHT) / 100.f),(player.getPosition().y - 50.f) }, 0, _gameData.renderTextureRed);
					Utils::BlitSprite(playerRun[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x - (TROAD_GAP * (-1 * (800.f - 50.f) + WORKING_HEIGHT) / 100.f),(player.getPosition().y - 50.f) }, 0, _gameData.renderTextureBlue);
					break;
				case 3:
					Utils::BlitSprite(playerRun[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x + (TROAD_GAP * (-1 * (800.f + 50.f) + WORKING_HEIGHT) / 100.f),(player.getPosition().y + 50.f) }, 0, _gameData.renderTextureRed);
					Utils::BlitSprite(playerRun[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x - (TROAD_GAP * (-1 * (800.f + 50.f) + WORKING_HEIGHT) / 100.f),(player.getPosition().y + 50.f) }, 0, _gameData.renderTextureBlue);
	...
	else
	{
		if (_gameData.tabPlayers.size() == 1)
		{
			...
			Utils::BlitSprite(playerIdle[_gameData.tabPlayers[0]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(playerIdle[_gameData.tabPlayers[0]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureBlue);
		}
		else
		{
			for (int i = _gameData.tabPlayers.size() - 1; i >= 0; i--)
			{
				...
				{
				case 0:
					Utils::BlitSprite(playerIdle[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ (player.getPosition().x - 50.f) + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureRed);
					Utils::BlitSprite(playerIdle[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ (player.getPosition().x - 50.f) - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureBlue);
					break;
				case 1:
					Utils::BlitSprite(playerIdle[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ (player.getPosition().x + 50.f) + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureRed);
					Utils::BlitSprite(playerIdle[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ (player.getPosition().x + 50.f) - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),player.getPosition().y }, 0, _gameData.renderTextureBlue);
				...
				case 2:
					Utils::BlitSprite(playerIdle[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x + (TROAD_GAP * (-1 * (800.f - 50.f) + WORKING_HEIGHT) / 100.f),(player.getPosition().y - 50.f) }, 0, _gameData.renderTextureRed);
					Utils::BlitSprite(playerIdle[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x - (TROAD_GAP * (-1 * (800.f - 50.f) + WORKING_HEIGHT) / 100.f),(player.getPosition().y - 50.f) }, 0, _gameData.renderTextureBlue);
					break;
				case 3:
					Utils::BlitSprite(playerIdle[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x + (TROAD_GAP * (-1 * (800.f + 50.f) + WORKING_HEIGHT) / 100.f),(player.getPosition().y + 50.f) }, 0, _gameData.renderTextureRed);
					Utils::BlitSprite(playerIdle[_gameData.tabPlayers[i]->GetPlayerSelected()], sf::Vector2f{ player.getPosition().x - (TROAD_GAP * (-1 * (800.f + 50.f) + WORKING_HEIGHT) / 100.f),(player.getPosition().y + 50.f) }, 0, _gameData.renderTextureBlue);
	...
	for (int i = 0; i < NB_LEVEL; i++)
	{
		if (playerPos == posLevel[i] || playerPos.x >= posLevel[i].x - 10 && playerPos.x <= posLevel[i].x + 10 && playerPos.y >= posLevel[i].y - 10 && playerPos.y <= posLevel[i].y + 10)
		{
			Utils::BlitSprite(nameLevel[i], sf::Vector2f{ nameLevel[i].getPosition().x + (TROAD_GAP * (-1 * 750.f + WORKING_HEIGHT) / 100.f),nameLevel[i].getPosition().y }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(nameLevel[i], sf::Vector2f{ nameLevel[i].getPosition().x - (TROAD_GAP * (-1 * 750.f + WORKING_HEIGHT) / 100.f),nameLevel[i].getPosition().y }, 0, _gameData.renderTextureBlue);
		}
	}
}

void MapManager::InitPlayerAnim(GameData& _gameData)
{
	for (int i = 0; i < 4; i++)
	{
...
void MapManager::ReloadForGameOver(GameData& _gameData)
{
	travelCaravanAnim->currentFrame = 0;
	travelLevel2Anim->currentFrame = 0;
	travelLevel3Anim->currentFrame = 0;
	caravanAnim->currentFrame = 0;
	boarAnim->currentFrame = 0;
	witchAnim->currentFrame = 0;
	level2Anim->currentFrame = 0;
	level3Anim->currentFrame = 0;
	_gameData.lastPosInMap = posLevel[1];
	player.setPosition(posLevel[1]);
	playerGoOn = NONEMAP;
	playerIsOn = MAP0;
	timer = 0;
