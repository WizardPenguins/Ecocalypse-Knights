...
void Paralax::Init(GameData& _gameData)
{
	...
	backGroundLevel3Boss[0] = Utils::LoadSprite("Assets/Images/Background/Map3/WideMap1Level3Boss.png", TOP_LEFT);
	backGroundLevel3Boss[1] = Utils::LoadSprite("Assets/Images/Background/Map3/WideMap2Level3Boss.png", TOP_LEFT);
	backGroundLevel3Boss[2] = Utils::LoadSprite("Assets/Images/Background/Map3/WideMap3Level3Boss.png", TOP_LEFT);
	backGroundLevel3Boss[3] = Utils::LoadSprite("Assets/Images/Background/Map3/WideMap7Level3Boss.png", TOP_LEFT);
	backGroundLevel3Boss[4] = Utils::LoadSprite("Assets/Images/Background/Map3/WideMap8Level3Boss.png", TOP_LEFT);



	maskTreeLevel3 = Utils::LoadSprite("Assets/Images/Background/Map3/MaskTree.png", TOP_LEFT);
	maskTreeLevel3.setPosition(7385.f, 1.f);
	...
	for (int i = 0; i < 5; i++)
	{
		backGroundLevel3Boss[i].setPosition(WORKING_WIDTH * 5.f, 3504.f);
	}
	...
	buildingThirdPlanSecondLevelAnim[CoruptNut].setPosition({ 1600,530 });
	...
	bridge.setPosition(9634.f, 1104.f);

	crack[0] = Utils::LoadSprite("Assets/Images/Background/Map3/cracks1.png", TOP_LEFT, 11, 1);
	crack[1] = Utils::LoadSprite("Assets/Images/Background/Map3/cracks2.png", TOP_LEFT, 11, 1);

	crackAnimation[0] = new Animation;
	crackAnimation[0]->CreateLeftToRight(crack[0], 11, 1, { 3740,504 });

	crackAnimation[1] = new Animation;
	crackAnimation[1]->CreateLeftToRight(crack[1], 11, 1, { 3421,664 });

	crack[0].setPosition(9475.f, 1350.f);
	crack[1].setPosition(11380.f, 1360.f);

	startPosEasings = bridge.getPosition();
	endPosEasings = { 9634.f ,4700.f };

	fallingBackGround = Utils::LoadSprite("Assets/Images/Background/Map3/FallingBackground.png", TOP_LEFT, 4, 1);
	fallingBackGround.setPosition(WORKING_WIDTH * 5.f, 1104.f + WORKING_HEIGHT);

	fallingBackGroundAnimation = new Animation;
	fallingBackGroundAnimation->CreateLeftToRight(fallingBackGround, 4, 1, { 7680,1080 });

	mask = Utils::LoadSprite("Assets/Images/Background/Map3/Mask.png", TOP_LEFT);
	mask2 = mask;
	mask.scale(6.5f, 6.5f);
	mask2.scale(-6.5f, 6.5f);

	startPosMask = { WORKING_WIDTH * 5.f,  WORKING_HEIGHT };
	endPosMask = { WORKING_WIDTH * 5.f,-WORKING_HEIGHT - 525.f };

	startPosMask2 = { WORKING_WIDTH * 5.f,  WORKING_HEIGHT * 2.f - 150.f };
	endPosMask2 = { WORKING_WIDTH * 5.f,-675.f };
}
...
void Paralax::Reload(GameData& _gameData)
{
	...
	if (_gameData.levelState == LEVEL3)
	{
		...
		_gameData.birdSpawn = false;
		_gameData.witchSpawn = false;
	}
	bridge.setPosition(9634.f, 1104.f);
	crackAnimation[0]->currentFrame = 0;
	crackAnimation[1]->currentFrame = 0;

	timerBridge = 0.f;
	timerMask = 0.f;
	_gameData.bridgeStartBroken = false;
}
...
void Paralax::UpdateAnim(GameData& _gameData)
{
	...
	case LEVEL3:
	{
		if (_gameData.actualWave == WAVE6 && _gameData.tabMob.size() == 0 && !_gameData.tabPlayers[0]->GetFreeFall())
		{
			_gameData.bridgeStartBroken = true;
			for (int i = 0; i < 2; i++)
			{
				crackAnimation[i]->UpdateLeftToRight(crack[i], _gameData.dt, 5, 0, temp, false);
			}
		}
		else
		{
		...
		if (crackAnimation[0]->currentFrame == crackAnimation[0]->maxFrame)
		{
			if (!isBridgeBroken)
			{
				_gameData.audio->PlaySoundNotRepeated("BRIDGE");
				isBridgeBroken = true;
			}
			for (int i = 0; i < _gameData.tabPlayers.size(); i++)
			{
				if (!_gameData.tabPlayers[i]->GetFreeFall() && !_gameData.birdSpawn)
				{
					_gameData.bridgeStartBroken = false;
					_gameData.tabPlayers[i]->SetFreeFall(true);
					_gameData.mobRespawn = true;
					_gameData.inTravel = false;
					_gameData.birdSpawn = true;

				}
			}
			timerBridge += _gameData.dt / 2.f;
			if (timerBridge < 1)
			{
				bridge.setPosition(Easings::Lerp(startPosEasings, endPosEasings, Easings::InBack(timerBridge)));
			...
			timerMask += _gameData.dt / 1.75f;
			if (timerMask < 1)
			{
				mask.setPosition(Easings::Lerp(startPosMask, endPosMask, Easings::InSine(timerMask)));
			}
		}
		if (_gameData.birdSpawn && !_gameData.witchSpawn && !_gameData.tabPlayers[0]->GetFreeFall())
		{
			_gameData.mobRespawn = true;
			_gameData.inTravel = false;
			_gameData.witchSpawn = true;

		}
		if (_gameData.tabPlayers[0]->GetFreefallStartCinematicBoss())
		{
			timerMask2 += _gameData.dt / 2.f;
			if (timerMask2 < 1)
			{
				mask.setPosition(Easings::Lerp(startPosMask2, endPosMask2, Easings::InSine(timerMask2)));
			}
		}
		fallingBackGroundAnimation->UpdateLeftToRight(fallingBackGround, _gameData.dt, 15, 0, temp, true);
	}
...
void Paralax::DisplayLevel3(GameData& _gameData)
{
	...
	sf::Vector2f baseScaleSrinks = { 1.5f * SHRINKS,1.5f * SHRINKS };
	for (int i = 0; i < _gameData.tabPlayers.size(); i++)
	{
		if (_gameData.tabPlayers[i]->playerIsShrinks)
		{
			float echelleMinimale = (baseScaleSrinks.y / 1.6f);
			float echelleMaximale = baseScaleSrinks.y;
			...
			if (_gameData.tabPlayers[i]->GetInAir())
			{
				...
				float hauteurSaut = _gameData.tabPlayers[i]->GetPos().y - (JUMPDISTANCE * SHRINKS);
			...
			((bool)(_gameData.tabPlayers[i]->GetLastDir() == LEFT) ?
				Utils::BlitSprite(_gameData.shadow, { _gameData.tabPlayers[i]->GetPos().x + (10 * SHRINKS) , _gameData.tabPlayers[i]->GetPos().y }, 0, *_gameData.rtSrinks) :
				Utils::BlitSprite(_gameData.shadow, { _gameData.tabPlayers[i]->GetPos().x - (10 * SHRINKS), _gameData.tabPlayers[i]->GetPos().y }, 0, *_gameData.rtSrinks));
		}
		_gameData.window.draw(_gameData.rtShadowSrinks);
	...
	if (_gameData.actualWave == WAVE6 && _gameData.tabMob.size() == 0)
	{
		for (int i = 0; i < 2; i++)
		{
			_gameData.window.draw(crack[i]);
		}
	}

	_gameData.window.draw(backGroundLevel3Boss[4]);
	_gameData.window.draw(backGroundLevel3Boss[3]);
	_gameData.window.draw(backGroundLevel3Boss[2]);
	_gameData.window.draw(backGroundLevel3Boss[1]);

	if (_gameData.tabPlayers[0]->GetFreeFall())
	{
		_gameData.window.draw(fallingBackGround);
	}
...
void Paralax::Display3DLevel3(GameData& _gameData)
{
	...
	Utils::BlitSprite(backGroundLevel3[1], sf::Vector2f{ backGroundLevel3[1].getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),backGroundLevel3[1].getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(backGroundLevel3[1], sf::Vector2f{ backGroundLevel3[1].getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),backGroundLevel3[1].getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(bridge, sf::Vector2f{ bridge.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), bridge.getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(bridge, sf::Vector2f{ bridge.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), bridge.getPosition().y }, 0, _gameData.renderTextureBlue);

	if (_gameData.actualWave == WAVE6 && _gameData.tabMob.size() == 0)
	{
		for (int i = 0; i < 2; i++)
		{
			Utils::BlitSprite(crack[i], sf::Vector2f{ crack[i].getPosition().x + (TROAD_GAP * (-1 * crack[i].getPosition().y + WORKING_HEIGHT) / 100.f), crack[i].getPosition().y }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(crack[i], sf::Vector2f{ crack[i].getPosition().x - (TROAD_GAP * (-1 * crack[i].getPosition().y + WORKING_HEIGHT) / 100.f), crack[i].getPosition().y }, 0, _gameData.renderTextureBlue);
		}
	}

	Utils::BlitSprite(backGroundLevel3Boss[4], sf::Vector2f{ backGroundLevel3Boss[4].getPosition().x + (TROAD_GAP * (-1 * backGroundLevel3Boss[4].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[4].getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(backGroundLevel3Boss[4], sf::Vector2f{ backGroundLevel3Boss[4].getPosition().x - (TROAD_GAP * (-1 * backGroundLevel3Boss[4].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[4].getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(backGroundLevel3Boss[3], sf::Vector2f{ backGroundLevel3Boss[3].getPosition().x + (TROAD_GAP * (-1 * backGroundLevel3Boss[3].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[3].getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(backGroundLevel3Boss[3], sf::Vector2f{ backGroundLevel3Boss[3].getPosition().x - (TROAD_GAP * (-1 * backGroundLevel3Boss[3].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[3].getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(backGroundLevel3Boss[2], sf::Vector2f{ backGroundLevel3Boss[2].getPosition().x + (TROAD_GAP * (-1 * backGroundLevel3Boss[2].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[2].getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(backGroundLevel3Boss[2], sf::Vector2f{ backGroundLevel3Boss[2].getPosition().x - (TROAD_GAP * (-1 * backGroundLevel3Boss[2].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[2].getPosition().y }, 0, _gameData.renderTextureBlue);

	Utils::BlitSprite(backGroundLevel3Boss[1], sf::Vector2f{ backGroundLevel3Boss[1].getPosition().x + (TROAD_GAP * (-1 * backGroundLevel3Boss[1].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[1].getPosition().y }, 0, _gameData.renderTextureRed);
	Utils::BlitSprite(backGroundLevel3Boss[1], sf::Vector2f{ backGroundLevel3Boss[1].getPosition().x - (TROAD_GAP * (-1 * backGroundLevel3Boss[1].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[1].getPosition().y }, 0, _gameData.renderTextureBlue);

	if (_gameData.tabPlayers[0]->GetFreeFall())
	{
		Utils::BlitSprite(fallingBackGround, sf::Vector2f{ fallingBackGround.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), fallingBackGround.getPosition().y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(fallingBackGround, sf::Vector2f{ fallingBackGround.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f), fallingBackGround.getPosition().y }, 0, _gameData.renderTextureBlue);
	}
}
...
void Paralax::DisplayFirstPlanLevel3(GameData& _gameData)
{
	...
	_gameData.window.draw(backGroundLevel3Boss[0]);

	if (_gameData.tabPlayers[0]->GetFreeFall())
	{
		_gameData.window.draw(mask);
	}
	if (_gameData.tabPlayers[0]->GetFreefallStartCinematicBoss())
	{
		_gameData.window.draw(mask2);
	}
}

void Paralax::DisplayFirstPlanLevel3(sf::RenderTexture& _red, sf::RenderTexture& _blue, bool _temp)
{
	Utils::BlitSprite(backGroundLevel3[0], sf::Vector2f{ backGroundLevel3[0].getPosition().x + (TROAD_GAP * (-1 * backGroundLevel3[0].getPosition().y + WORKING_HEIGHT) / 100.f),backGroundLevel3[0].getPosition().y }, 0, _red);
	Utils::BlitSprite(backGroundLevel3[0], sf::Vector2f{ backGroundLevel3[0].getPosition().x - (TROAD_GAP * (-1 * backGroundLevel3[0].getPosition().y + WORKING_HEIGHT) / 100.f),backGroundLevel3[0].getPosition().y }, 0, _blue);

	if (_temp) // rajouter la condition que le joueur est draw derrière
	{
	...
	Utils::BlitSprite(backGroundLevel3Boss[0], sf::Vector2f{ backGroundLevel3Boss[0].getPosition().x + (TROAD_GAP * (-1 * backGroundLevel3Boss[0].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[0].getPosition().y }, 0, _red);
	Utils::BlitSprite(backGroundLevel3Boss[0], sf::Vector2f{ backGroundLevel3Boss[0].getPosition().x - (TROAD_GAP * (-1 * backGroundLevel3Boss[0].getPosition().y + WORKING_HEIGHT) / 100.f), backGroundLevel3Boss[0].getPosition().y }, 0, _blue);
}
...
void Paralax::DisplayFirstPlan(sf::RenderTexture& _red, sf::RenderTexture& _blue, LevelState _level, bool _temp)
{
	switch (_level)
	{
	case LEVEL1:
	...
	case LEVEL3:
	{
		DisplayFirstPlanLevel3(_red, _blue, _temp);
		break;
	}
	default:
		break;
