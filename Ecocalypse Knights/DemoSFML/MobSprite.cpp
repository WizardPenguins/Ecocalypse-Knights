...
void BoarSprite::InitStun(SpriteAnim& _sprite)
{
	std::fstream infile("Assets/Images/Entities/Mobs/param.txt", std::ios::in);
		...
		if (infile.is_open())
		{
			while (!infile.eof())
			{
				...
				if (current == "Stun")
				{
					...
					_sprite.sprite = Utils::LoadSprite("Assets/Images/Entities/Mobs/Boar/Stun.png", CENTER_FOOT, (int)_sprite.nbAnim.x, (int)_sprite.nbAnim.y);
					infile.close();
					return;
				}
				else
