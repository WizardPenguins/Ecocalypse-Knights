...
void States::DisplayButtons3D(sf::RenderTexture& _red, sf::RenderTexture& _blue)
{
	for (unsigned char i = 0; i < buttonVect.size(); i++)
	{
		Utils::BlitSprite(buttonVect[i]->sprite, sf::Vector2f{ buttonVect[i]->sprite.getPosition().x + (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),buttonVect[i]->sprite.getPosition().y }, 0, _red);
		Utils::BlitSprite(buttonVect[i]->sprite, sf::Vector2f{ buttonVect[i]->sprite.getPosition().x - (TROAD_GAP * (-1 * 800.f + WORKING_HEIGHT) / 100.f),buttonVect[i]->sprite.getPosition().y }, 0, _blue);
	}
}

void States::RightLeftKeysButtons(GameData& _gameData)
{
