...
void Camera::Move(GameData& _gameData)
{
	...
	else if (_gameData.tabPlayers[0]->GetFreeFall() && _gameData.tabPlayers[0]->GetFreefallEndCinematicBird() && !_gameData.tabPlayers[0]->GetFreefallStartCinematicBoss())
	{
		center = { WORKING_WIDTH * 5.5f,  1104.f + WORKING_HEIGHT *1.5f};
		ratio = 1920.f;
	}
	else if (_gameData.tabPlayers[0]->GetFreeFall() && !_gameData.tabPlayers[0]->GetFreefallEndCinematicBird() && !_gameData.tabPlayers[0]->GetFreefallStartCinematicBoss())
	{
	...
	else if (_gameData.tabPlayers[0]->GetFreeFall() && _gameData.tabPlayers[0]->GetFreefallStartCinematicBoss())
	{
		speed = { 450.f ,1000.f };
		center = { WORKING_WIDTH * 5.5f, 4065.f };
		ratio = 1920.f;
	}
	else if (_gameData.witchSpawn)
	{
		speed = { 450.f ,800.f };
		center = { WORKING_WIDTH * 5.5f, 4045.f };
		ratio = 1920.f;
	}
	else
	{
		...
		if (!_gameData.inTravel)
		{
			...
			else if (levelState == LEVEL3 && actualWave < WAVE6)
			{
				if (center.x - size.x / 2.f < WORKING_WIDTH * actualWave)
				{
					center.x = size.x / 2.f + WORKING_WIDTH * actualWave;
				...
				if (center.y - size.y / 2.f < LEVEL3_GAP)
				{
					center.y = size.y / 2.f+ LEVEL3_GAP;
				}
				else if (center.y + size.y / 2.f > WORKING_HEIGHT+ LEVEL3_GAP)
				{
					center.y = (WORKING_HEIGHT + LEVEL3_GAP) - size.y / 2.f;
				}
			}
...
{
	if (_player->GetPos().x >= 2105.f && _player->GetPos().x <= 2205.f)
	{
		if (_player->GetState() != DASH && _player->GetState() != IDLE)
		{
			_player->SetPos({ _player->GetPos().x , 4075.f });
	...
	if (_player->GetPos().x >= 2105.f && _player->GetPos().x <= 3400.f)
	{
	...
	if (_playerIsShrinks && _player->GetPos().x <= 2305.f)
	{
		if (_player->GetInAir())
		{
			_player->SetInAir(false);
...
void Camera::Update(GameData& _gameData)
{
	...
	if (!_gameData.inCinematic && !_gameData.tabPlayers[0]->GetFreeFall())
	{
...
void Camera::DisplayFirstPlan(sf::RenderTexture& _red, sf::RenderTexture& _blue, LevelState _level,bool _temp)
{
	paralax.DisplayFirstPlan(_red, _blue, _level,_temp);
}
...
void Camera::SetGoTo(sf::Vector2f _goTo,GameData& _gameData)
{
	goTo = _goTo;
	if (goTo != lastCenter)
	{
