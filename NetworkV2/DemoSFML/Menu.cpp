...
void Menu::Display3D(GameData& _gameData)
{
	...
	if (!_gameData.multiplayer)
	{
		...
		Utils::BlitSprite(spriteGameName, sf::Vector2f{ spriteGameName.getPosition().x + (TROAD_GAP * (-1 * spriteGameName.getPosition().y + WORKING_HEIGHT) / 100.f),spriteGameName.getPosition().y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(spriteGameName, sf::Vector2f{ spriteGameName.getPosition().x - (TROAD_GAP * (-1 * spriteGameName.getPosition().y + WORKING_HEIGHT) / 100.f),spriteGameName.getPosition().y }, 0, _gameData.renderTextureBlue);

		if (currentPhase == PhaseType::PHASE1 || currentPhase == PhaseType::PHASE2 || currentPhase == PhaseType::PHASE3)
		{
			Utils::BlitSprite(_gameData.swordSpriteMenu, sf::Vector2f{ _gameData.swordSpriteMenu.getPosition().x + (TROAD_GAP * (-1 * _gameData.swordSpriteMenu.getPosition().y + WORKING_HEIGHT) / 100.f),_gameData.swordSpriteMenu.getPosition().y }, 0, _gameData.renderTextureRed);
			Utils::BlitSprite(_gameData.swordSpriteMenu, sf::Vector2f{ _gameData.swordSpriteMenu.getPosition().x - (TROAD_GAP * (-1 * _gameData.swordSpriteMenu.getPosition().y + WORKING_HEIGHT) / 100.f),_gameData.swordSpriteMenu.getPosition().y }, 0, _gameData.renderTextureBlue);
		}
	}
