...
{
	if (this->status != Status::OnIce && this->status != Status::OnElec)
	{
		switch (this->race)
		{
		case RaceMob::HazelNut:
...
{
	if (this->status != Status::OnIce && this->status != Status::OnElec && this->inAir)
	{
		switch (this->race)
		{
		case RaceMob::HazelNut:
...
{
	if (this->status != Status::None && !this->inAir)
	{
...
void Mob::OutOfScreen(Wave _wave)
{
	if (this->boar == nullptr && this->witch == nullptr)
	{
		if (this->pos.x < 0 + (int)(_wave) * 1920.f)
		{
			...
			if (this->pig != nullptr)
			{
				this->pig->SetState(PigState::Target);
			}
		}
