...
void Pause::Display3D(GameData& _gameData)
{
	...
	if (_gameData.lastState == GAME && _gameData.levelState == MAP)
	{
		Utils::BlitSprite(_gameData.swordSpriteMenu, sf::Vector2f{ swordCurrentPos.x + (TROAD_GAP * (-1 * swordCurrentPos.y + WORKING_HEIGHT) / 100.f),swordCurrentPos.y }, 0, _gameData.renderTextureRed);
		Utils::BlitSprite(_gameData.swordSpriteMenu, sf::Vector2f{ swordCurrentPos.x - (TROAD_GAP * (-1 * swordCurrentPos.y + WORKING_HEIGHT) / 100.f),swordCurrentPos.y }, 0, _gameData.renderTextureBlue);
	}
