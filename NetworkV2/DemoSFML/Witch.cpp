...
Witch::Witch(Mob* _mob, WitchSprite* _sprite, Wave _wave)
{
	//variable mob
	this->mob = _mob;
	this->mob->pos = { (float)(1550.f + _wave * 1980.f),850.f };
...
void Witch::CreateNewPlant(Wave _wave)
{
	...
	temp->pos = sf::Vector2f{ (float)(((rand() % 1690) + 200.f) + (1980.f * _wave)),(float)((rand() % 360) + 700.f) };
...
void Witch::InitCystal()
{
	for (int i = 0; i < NbCrystal; i++)
	{
		...
		this->crystal[i].pos = { this->mob->pos.x - 300.f,700.f + (300.f * i) };
...
void Witch::AddBramble(float _playerY)
{
	...
	{
		temp = new Bramble{ this->sprite.spriteBrambleInvoc,nullptr,sf::Vector2f{this->mob->pos.x + 300.f,_playerY},true,false };
	}
	else
	{
...
void Witch::UpdateCrystalHit(float _dt, Audio& _audio)
{
	...
	{
		_audio.PlaySoundRepeated("WITCH_CRISTAL_SOUND_AMBIANCE_1", 100.f);
...
void Witch::Death(Audio& _audio)
{
	...
	{
		_audio.PlaySoundNotRepeated("WITCH_DEATH_SCREAM_1", 100.f);
		_audio.PlaySoundNotRepeated("WITCH_DEATH_SOUND_1", 100.f);
