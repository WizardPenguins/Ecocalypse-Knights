...
sf::Packet& operator >>(sf::Packet& _packet, Wave& _wave)
{
	int temp;
	_packet >> temp;
	_wave = (Wave)temp;
...
sf::Packet& operator >>(sf::Packet& _packet, LevelState& _state)
{
	int temp;
	_packet >> temp;
	_state = (LevelState)temp;
...
void Server::UpdateServer(GameData& _gameData, sf::Vector2f& _swordPos)
{
	// Attente d'une communication sur le selector
	//if (this->selector.wait())
	{
		if (this->clients.size() > 0)
		{
			this->SendReceive(_gameData, _swordPos);
		}
	}
}
...
void Server::SendReceive(GameData& _gameData, sf::Vector2f& _swordPos)
{
	if (!this->selector.isReady(this->listener))
	{
		for (int i = 0; i < this->clients.size(); i++)
		{
			...
			//if (this->clients.at(i)->receive(this->packet) == sf::Socket::Done)
			{
				...
				for (int j = 0; j < _gameData.tabPlayers.size(); j++)
				{
					this->packet << _gameData.gameState << _swordPos;
				}
				this->clients.at(i)->send(this->packet);
			}
		}
...
void Server::SendReceive(GameData& _gameData, LevelState* _levelState)
{
	if (!this->selector.isReady(this->listener))
	{
		for (int i = 0; i < this->clients.size(); i++)
		{
			...
			{
				//int temp;
				this->packet >> *_gameData.tabPlayers[i + 1];
				//if (*_levelState != temp)
				//{
				//	/*if (*_levelState == MAP)
				//	{
...
void ReadPacket(sf::Packet& _packet, std::vector<Player*>* _player, int _id, GameData& _gameData, sf::Vector2f& _swordPos)
{
	for (int i = 0; i < _player->size(); i++)
	{
		if (i == _id)
		{
			// Ecrasement des donnees du joueurs operant
			sf::Vector2f trashPos;
			int trash;

			_packet >> trash >> trashPos;
		}
		else
		{
			_packet >> _gameData.gameState >> _swordPos;
		}
	}
	_packet.clear();
}
...
void ReadPacket(sf::Packet& _packet, std::vector<Player*>* _player, int _id, GameData& _gameData, std::vector<PlayerSkins>* _tabPlayerSkin)
{
	for (int i = 0; i < _player->size(); i++)//on all player
	{
		...
		else
		{
			...
			int nbPlayer;
			_packet >> nbPlayer >> _tabPlayerSkin->at(i);
			if (nbPlayer > 0 && nbPlayer <= 4)
			{
				_gameData.network->client->nbPlayer = nbPlayer;
			}
			...
			{
				_gameData.ChangeState(tempState);
...
void Network::SetEnteredCharServer(sf::Event* _event)
{
	if (_event->type == sf::Event::TextEntered)
	{
		...
		if (_event->text.unicode == 8 && this->tabStr.size() > 0)
		{
...
void Network::InitRectangle()
{
	...
	this->bgServerText->setPosition(WORKING_WIDTH / 2 - 100, 200 + GAP);
...
void Network::InitButton()
{
	...
	this->tabButton[0]->setPosition(WORKING_WIDTH / 2 - 125, WORKING_HEIGHT / 3 + GAP + 75);
	this->tabButton[1]->setPosition(WORKING_WIDTH / 2 - 125, WORKING_HEIGHT / 1.5 + GAP - 10);
...
Network::Network()
{
	...
	hostSprite = Utils::LoadSprite("Assets/Images/Menu/Host.png", TOP_LEFT);
	clientSprite = Utils::LoadSprite("Assets/Images/Menu/Client.png", TOP_LEFT);
	confirmSprite = Utils::LoadSprite("Assets/Images/Menu/Confirm.png", TOP_LEFT);
	IpAddress = Utils::LoadSprite("Assets/Images/Menu/IPSelected.png", TOP_LEFT);
	confirmSprite.setOrigin({ 0, 50 });
	confirmSprite.setPosition(WORKING_WIDTH / 2 - 50, WORKING_HEIGHT - 200 + GAP);
	IpAddress.setPosition(WORKING_WIDTH / 2 - 300, 220 + GAP - 100);
}
...
void Network::IngameStateUpdatePause(GameData& _gameData, sf::Vector2f& _swordPos)
{
	if (this->state == NetworkState::HOST)//server
	{
		server->UpdateServer(_gameData, _swordPos);//send receive data
	}
	else if (this->state == NetworkState::CLIENT)//client
	{
		//LoadPacket(this->packet, _gameData.tabPlayers[this->client->id]);//add the current client to the packet
		...
		this->client->ReceiveToServer2(this->packet);//just receive it
		ReadPacket(this->packet, &_gameData.tabPlayers, this->client->id, _gameData, _swordPos);//read data inside
...
void Network::PendingDisplay(sf::RenderWindow* _window, Camera* _camera, sf::Sprite* _flame)
{
	...
	_window->draw(*_flame);
	hostSprite.setPosition(tabButton[0]->getPosition());
	clientSprite.setPosition(tabButton[1]->getPosition());
	_window->draw(hostSprite);
	_window->draw(clientSprite);
}
...
void Network::ClientWaitDisplay(sf::RenderWindow* _window, Camera* _camera, sf::Sprite* _flame)
{
	_window->draw(spriteBackground);
	_window->draw(*_flame);
	_window->draw(confirmSprite);
	_window->draw(IpAddress);
	if (isRevealed)
	{
		_window->draw(*this->tabText[0]);
	}
...
void Network::SendMob(GameData& _gameData)
{
	int size = (int)_gameData.tabMob.size();
	int sizeCircle = (int)_gameData.levelManager->vectorCircle.size();
	this->packet << "TabMob" << _gameData.mobRespawn << size << sizeCircle;
	//this->packet << _gameData.mobRespawn << size << sizeCircle;
	for (int i = 0; i < size; i++)
	{
		sf::Vector2f pos = _gameData.tabMob[i]->GetPos();
...
void Network::ParserNetworkSend(GameData& _gameData, Data& _data, int _id)
{
	if (_gameData.gameState == GAME)
	{
		SendMob(_gameData);
		this->packet << "ActualWave" << _gameData.actualWave;
		this->packet << "LevelState" << _data.state;
	}
	this->packet << "GameState" << _gameData.gameState;
	if (_gameData.gameState == PAUSE)
	{
		this->packet << "SwordPos" << _data.swordPos;
	}
	if (this->state == NetworkState::CLIENT)
	{
		this->packet << "PlayerID" << _id;
		this->packet << "NBPlayer" << this->client->nbPlayer;
		if (_gameData.gameState == CHARACTER_SELECTION)
		{
			this->packet << "Player" << this->client->id;
			this->packet << "PlayerSkins" << _data.tabSkin->at(this->client->id);
		}
		this->packet << "Player" << this->client->id;
		this->packet << "PlayerData" << _gameData.tabPlayers[this->client->id];
	}
	else
	{
		this->packet << "PlayerID" << _id;
		this->packet << "NBPlayer" << this->server->nbPlayer;
		for (int i = 0; i < _gameData.tabPlayers.size(); i++)
		{
			if (_gameData.gameState == CHARACTER_SELECTION)
			{
				this->packet << "Player" << i;
				this->packet << "PlayerSkins" << _data.tabSkin->at(i);
			}
			this->packet << "Player" << i;
			this->packet << "PlayerData" << _gameData.tabPlayers[i];
		}
	}
}

void Network::SynchroMob(GameData& _gameData, int _size)
{
...
void Network::SynchroDrop(GameData& _gameData, int _size)
{
	bool recreate = false;
	if (_size != _gameData.drop.size())
	{
		recreate = true;
		_gameData.drop.clear();
	}
...
void Network::SynchroCircle(GameData& _gameData, int _sizeCircle)
{
	for (int i = 0; i < _sizeCircle; i++)
	{
		packet >> _gameData.levelManager->vectorCircle[i].pos;
		_gameData.levelManager->vectorCircle[i].circleInvoc.setPosition(_gameData.levelManager->vectorCircle[i].pos);
	}
...
void Network::PlayerNetworkReceive(GameData& _gameData, int _i)
{
	std::string dataType;

	if (this->packet >> dataType)
	{
		if (dataType == "PlayerData")
		{
			Player* trash = new Player{ _gameData, 8 };
			if ((_i == this->client->id) || (this->state == NetworkState::HOST && _i == 0))
			{
				this->packet >> *trash;
			}
			else
			{
				this->packet >> *_gameData.tabPlayers[_i];
			}
		}
		else if (dataType == "PlayerSkins")
		{
			std::vector<PlayerSkins> trash;
			PlayerSkins skin;
			trash.push_back(skin);
			if ((_i == this->client->id) || (this->state == NetworkState::HOST && _i == 0))
			{
				this->packet >> trash[_i];
			}
			else
			{
				this->packet >> data.tabSkin->at(_i);
			}
		}
		else
		{
...
void Network::ParserNetworkReceive(GameData& _gameData)
{
	std::string dataType;

	while (this->packet >> dataType)
	{
		if (this->state == NetworkState::CLIENT)
		{
			int trash;
			if (dataType == "TabMob")
			{
				...
				SynchroMob(_gameData, size);
				SynchroCircle(_gameData, sizeCircle);
				packet >> size;
				SynchroDrop(_gameData, size);
			}
			...
			else if (dataType == "GameState")
			{
				this->packet >> _gameData.gameState;
			}
			else if (dataType == "SwordPos")
			{
				this->packet >> *data.swordPos;
			}
			else if (dataType == "ActualWave")
			{
				this->packet >> _gameData.actualWave;
			}
			else if (dataType == "LevelState")
			{
				this->packet >> *data.state;
			}
			else if (dataType == "PlayerID")
			{
				this->packet >> this->client->id;
			}
			else if (dataType == "NBPlayer")
			{
				this->packet >> this->client->nbPlayer;
			}
			else
			{
				this->packet >> trash;
		...
		if (dataType == "Player")
		{
			int id;
			this->packet >> id;
			PlayerNetworkReceive(_gameData, id);
		}
	}
}

void Network::AcceptConnection(GameData& _gameData)
{
	//add it to the vector client and accept it from the listener
	sf::TcpSocket* socket = new sf::TcpSocket{};
	sf::Packet* newClient = new sf::Packet{};
	...
	this->server->listener.accept(*socket);
	this->server->selector.add(*socket);
	this->server->clients.push_back(socket);

	if (socket->receive(*newClient) == sf::Socket::Done)//receive data from the newPlayer
	{
		*newClient >> pseudo;//receive pseudo

		this->server->nbPlayer++; //up number player

		Player* temp = new Player(_gameData, this->server->nbPlayer - 1); //add it to the vector
		temp->Init();
		_gameData.tabPlayers.push_back(temp);
		...
		//packet << id of the player << number player
		*newClient << "PlayerID" << this->server->nbPlayer - 1;
		*newClient << "NBPlayer" << this->server->nbPlayer;
		*newClient << "Camera" << _gameData.camera->GetView().getCenter().x;
		//*newClient << this->server->nbPlayer - 1 << this->server->nbPlayer << _gameData.camera->GetView().getCenter().y;//send his ID and the number of player the ID correspond of the number player -1 because it start to 1
		socket->send(*newClient);
	}
...
void Network::UpdateParserReceive(GameData& _gameData)
{
	if (this->state == NetworkState::CLIENT)
	{
		if (this->client->socket.receive(this->packet) == sf::Socket::Done)
		{
			ParserNetworkReceive(_gameData);
		}
		while (_gameData.tabPlayers.size() < this->client->nbPlayer)
		{
			Player* temp2 = new Player(_gameData, 0);
	...
	{
		if (!this->server->selector.isReady(this->server->listener))
		{
			for (int i = 0; i < _gameData.tabPlayers.size(); i++)
			{
				if (this->server->clients.at(i)->receive(this->packet) == sf::Socket::Done)
				{
					ParserNetworkReceive(_gameData);
				}
			}
		...
		else //if the listener have a new entry
		{
			AcceptConnection(_gameData);
		}
		
		while (_gameData.tabPlayers.size() < this->server->nbPlayer)
		{
			Player* temp2 = new Player(_gameData, 0);
			temp2->Init();
			_gameData.tabPlayers.push_back(temp2);
...
void Network::UpdateParserSend(GameData& _gameData, Data& _data)
{
	if (this->state == NetworkState::CLIENT)
	{
		ParserNetworkSend(_gameData, _data, this->client->id);
	...
	else
	{
		...
		{
			ParserNetworkSend(_gameData, _data, i + 1);
			this->server->clients[i]->send(this->packet);
		}
		this->packet.clear();
	}
}

void Network::UpdateParser(GameData& _gameData, Data& _data)
{
	UpdateParserReceive(_gameData);
	UpdateParserSend(_gameData, _data);
}